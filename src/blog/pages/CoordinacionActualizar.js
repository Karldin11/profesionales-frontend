...
const CoordinacionActualizar = () => {
    ...
    const fetchPost = async () => {
      try {
        ...
        const responseData = await sendRequest(
          process.env.REACT_APP_BACKEND_URL + `/coordinacion/post/${postId}`
        );
  ...
  const postUpdateSubmitHandler = async (event) => {
    ...
    try {
      ...
      history.push("/");
      history.replace("coordinaciones");
    } catch (err) {}
  };
...
export default CoordinacionActualizar;
