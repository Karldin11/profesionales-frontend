...
const CoordinacionItem = (props) => {
  ...
  const confirmDeleteHandler = async () => {
    ...
    try {
      await sendRequest(
        process.env.REACT_APP_BACKEND_URL + `/coordinacion/post/${props.id}`,
        "DELETE"
      );
      ...
      <li className="item">
        <div className="item__content">
          ...
          <div className="item__actions">
            {auth.isLoggedIn && (
              <Button to={`/publicacion/coordinacion/${props.id}`}>
                EDITAR
              </Button>
            )}
...
export default CoordinacionItem;
