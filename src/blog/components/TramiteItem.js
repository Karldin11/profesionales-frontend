...
const TramiteItem = (props) => {
  ...
  const confirmDeleteHandler = async () => {
    ...
    try {
      await sendRequest(
        process.env.REACT_APP_BACKEND_URL + `/tramites/${props.id}`,
        "DELETE"
      );
      ...
      <li className="item">
        <div className="item__content">
          ...
          <div className="item__actions">
            {auth.isLoggedIn && (
              <Button to={`/publicacion/tramite/${props.id}`}>EDITAR</Button>
            )}

            <Button to={`/tramites/${props.id}`}>VER DETALLES</Button>
